# -*- coding: utf-8 -*-
"""
Build Script to generate HTML treatise pages for the 4 new manuscripts:
1. About Shri Moksham (about-shri-moksham.html, the-about-shri-moksham.html, about.html)
2. Atmabodh (atmabodh.html, the-atmabodh.html)
3. The Mind Is Not the Self (the-mind-is-not-the-self.html, mind-is-not-the-self.html)
4. Course Requirements & Outcomes (course-requirements-and-outcomes.html, the-course-requirements-and-outcomes.html, course-requirement-and-outcome.html)
"""
...
if __name__ == '__main__':
    all_treatises = [
        (about_shri_moksham_meta, ['the-about-shri-moksham.html', 'about.html']),
        (atmabodh_meta, ['the-atmabodh.html']),
        (the_mind_meta, ['mind-is-not-the-self.html']),
        (course_requirements_meta, ['the-course-requirements-and-outcomes.html', 'course-requirement-and-outcome.html'])
    ]

    for meta, aliases in all_treatises:
        primary_filename = meta['filename']
        html_code = render_treatise_page(meta)

        with open(primary_filename, 'w', encoding='utf-8') as f:
            f.write(html_code)
        print(f"Generated Primary: {primary_filename} ({len(html_code)} bytes)")

        for alias in aliases:
            # For alias files, ensure self-link or active link stays valid
            with open(alias, 'w', encoding='utf-8') as f:
                f.write(html_code)
            print(f"Generated Alias: {alias} ({len(html_code)} bytes)")

    print("\nAll 4 new manuscripts generated with complete aliases successfully!")
